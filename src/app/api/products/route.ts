    ...
    const scored = products
      .map((product) => {
        ...
        if (!tokens.length) {
          ...
          score += tokens.reduce((total, token) => total + (haystack.includes(token) ? 40 : 0), 0);
        }
      ...
      .sort((a, b) => {
        if (sort === "price") {
          return (a.product.listings[0]?.price ?? Number.MAX_SAFE_INTEGER) - (b.product.listings[0]?.price ?? Number.MAX_SAFE_INTEGER);
        }
