...
type Listing = { id: string; marketplace: string; price: number; seller: string | null; rating: number | null; reviewCount: number | null; url: string; inStock: boolean; lastCheckedAt: string };
...
export default function Home() {
  ...
  async function liveSearch() { if (!query.trim()) return; try { setLiveLoading(true); setMessage(""); setError(""); const response = await fetch(`/api/search/live?q=${encodeURIComponent(query.trim())}&limit=10`, { cache: "no-store" }); const data = await response.json(); if (!response.ok) throw new Error(data.error || "Live search failed"); const result = data as LiveResponse; setMessage(`Live search found ${result.stats.discovered} marketplace results and saved ${result.stats.upsertedListings} priced listings.`); await loadProducts(); } catch (err) { setError(err instanceof Error ? err.message : "Live search failed."); } finally { setLiveLoading(false); } }

  return <main className="page">
    <header className="header"><nav className="nav"><Link className="logo" href="/">compare.</Link><span className="badge">V2 · Universal Product Search</span></nav></header>
    <section className="hero"><div className="eyebrow">Universal Shopping Search</div><h1>Search once.<br />Compare everywhere.</h1><p className="sub">Search your product database or pull fresh marketplace results from Shopee, Tokopedia and Lazada.</p>
    ...
    <section className="content"><div className="section-title"><h2>{query ? `Results for “${query}”` : "Popular products"}</h2><span className="demo">{total} products</span></div>
      ...
      {loading && <div className="empty">Searching local catalog…</div>}{!loading && !error && <div className="grid">{products.map(product => { const sorted = [...product.listings].sort((a,b) => a.price-b.price); const best = sorted[0]; return <article className="card" key={product.id}><Link className="product-link" href={`/product?id=${encodeURIComponent(product.id)}`}><div className="product-head"><div className="thumb">{iconFor(product.category)}</div><div><h3 className="product-name">{product.name}</h3><div className="meta">{product.brand} · {product.category}{product.modelNumber ? ` · ${product.modelNumber}` : ""}</div></div></div></Link><div className="price-summary"><div><span className="meta">Best price</span><div className="price">{best ? money.format(best.price) : "—"}</div></div><span className="listing-count">{sorted.length} stores</span></div><div className="listings">{sorted.map((listing,i) => <div className="listing" key={listing.id}><div><div className="market">{listing.marketplace}{i===0 ? " · Best price" : ""}</div><div className="meta">{listing.seller ?? "Marketplace seller"}</div><div className="price">{money.format(listing.price)}</div></div><a className="buy" href={listing.url} target="_blank" rel="noreferrer">View store</a></div>)}</div><Link className="compare-link" href={`/product?id=${encodeURIComponent(product.id)}`}>Compare prices & history →</Link></article>; })}{!products.length && <div className="empty">No products matched. Enter a product above and use Search live to discover marketplace listings.</div>}</div>}
    </section>
  </main>;
}
