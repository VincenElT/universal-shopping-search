...
function marketplaceFromUrl(url: string): string { const host = new URL(url).hostname.toLowerCase().replace(/^www\./, ""); if (host === "shopee.co.id" || host.endsWith(".shopee.co.id")) return "shopee"; if (host === "tokopedia.com" || host.endsWith(".tokopedia.com")) return "tokopedia"; if (host === "lazada.co.id" || host.endsWith(".lazada.co.id")) return "lazada"; return host || "other"; }
...
function isProductUrl(listing: DiscoveredListing) {
  ...
  return listing.price != null && segments.length >= 1;
}

export function cleanAndDeduplicateListings(listings: DiscoveredListing[]) { const seen = new Set<string>(); return listings.flatMap((listing) => { if (!listing.marketplace || listing.marketplace === "other") return []; let url: string; try { url = normalizeUrl(listing.url); } catch { return []; } const normalizedListing = { ...listing, url }; if (!isProductUrl(normalizedListing)) return []; const key = `${listing.marketplace}:${url}`; if (seen.has(key)) return []; seen.add(key); return [normalizedListing]; }); }
