...
import { isProductUrl, marketplaceFromUrl, normalizeProductUrl } from "./product-url";
...
export function cleanAndDeduplicateListings(listings: DiscoveredListing[]) {
  ...
  return listings.flatMap((listing) => {
    if (!listing.marketplace || listing.marketplace === "other") return [];
    let url: string;
    try { url = normalizeProductUrl(listing.url); } catch { return []; }
...
export async function discoverListings(keyword: string, options?: { limit?: number }) {
  ...
  try {
    ...
    if (provider === "auto") { console.warn("[search] Gemini returned 0 usable listings; falling back to Serper."); return await discoverWithSerper(keyword, options); }
    console.warn("[search] Gemini returned 0 usable listings; explicit Gemini provider will not use Serper."); return [];
  } catch (error) {
