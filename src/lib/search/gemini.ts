...
import { isProductUrl } from "./product-url";
...
function marketplaceFromUrl(url: string): DiscoveredListing["marketplace"] { try { const host = new URL(url).hostname.toLowerCase().replace(/^www\./, ""); if (host === "shopee.co.id" || host.endsWith(".shopee.co.id")) return "shopee"; if (host === "tokopedia.com" || host.endsWith(".tokopedia.com")) return "tokopedia"; if (host === "lazada.co.id" || host.endsWith(".lazada.co.id")) return "lazada"; if (host === "google.com" || host.endsWith(".google.com") || host.endsWith(".googleapis.com") || host.endsWith("googleusercontent.com")) return "other"; return host; } catch {} return "other"; }
function isGoogleGroundingRedirect(url: string) { try { const parsed = new URL(url); const host = parsed.hostname.toLowerCase(); return host === "vertexaisearch.cloud.google.com" && parsed.pathname.startsWith("/grounding-api-redirect/"); } catch { return false; } }
async function resolveListingUrl(url: string) {
  if (!isGoogleGroundingRedirect(url)) return url;
  const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 5000);
  try { const response = await fetch(url, { method: "HEAD", redirect: "follow", signal: controller.signal, cache: "no-store", headers: { "User-Agent": "Mozilla/5.0 (compatible; UniversalShoppingSearch/1.0)" } }); const resolved = response.url; if (resolved && !isGoogleGroundingRedirect(resolved) && marketplaceFromUrl(resolved) !== "other") return resolved; } catch {} finally { clearTimeout(timeout); }
  const controller2 = new AbortController(); const timeout2 = setTimeout(() => controller2.abort(), 5000);
  try { const response = await fetch(url, { method: "GET", redirect: "follow", signal: controller2.signal, cache: "no-store", headers: { "User-Agent": "Mozilla/5.0 (compatible; UniversalShoppingSearch/1.0)" } }); const resolved = response.url; if (resolved && !isGoogleGroundingRedirect(resolved) && marketplaceFromUrl(resolved) !== "other") return resolved; } catch {} finally { clearTimeout(timeout2); }
  return null;
}
...
function marketplaceUrlFromText(text: string) { const urls = text.match(/https?:\/\/[^\s"'<>]+/gi) ?? []; return urls.find((url) => marketplaceFromUrl(url) !== "other"); }
function bestGroundingUrl(item: GeminiListing, chunks: GroundingChunk[]) { if (item.url) return item.url; const indexed = item.sourceIndex != null ? chunks[item.sourceIndex]?.web : undefined; if (indexed?.uri) return indexed.uri; return undefined; }
...
export async function discoverListingsWithGemini(keyword: string, options?: { limit?: number }) {
  ...
  const textUrl = marketplaceUrlFromText(text); const seen = new Set<string>(); const resolvedListings = await Promise.all(parsedListings.map(async (item) => {
    const rawUrl = bestGroundingUrl(item, chunks) ?? textUrl; if (!rawUrl || !item.title) return null; const resolvedUrl = await resolveListingUrl(rawUrl); if (!resolvedUrl) return null; let url: string; try { url = normalizeUrl(resolvedUrl); } catch { return null; }
    const marketplace = marketplaceFromUrl(url); if (marketplace === "other") return null; const rating = item.rating == null ? null : Number(item.rating); const reviewCount = parseCount(item.reviewCount); const soldCount = parseCount(item.soldCount); const safeRating = Number.isFinite(rating ?? NaN) ? rating : null; const price = parseMoney(item.price);
    if (!isProductUrl({ marketplace, url, price })) { console.warn("[gemini-debug] rejected non-product URL", { marketplace, url, title: item.title }); return null; }
  ...
  console.log("[gemini-debug] usable", { count: results.length, byMarketplace: Object.groupBy(results, (listing) => listing.marketplace) }); return results;
}
