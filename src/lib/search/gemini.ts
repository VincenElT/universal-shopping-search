...
type GeminiResponse = {
  candidates?: Array<{
    ...
    finishReason?: string;
    groundingMetadata?: { groundingChunks?: GroundingChunk[]; groundingSupports?: unknown[]; webSearchQueries?: string[]; searchEntryPoint?: { renderedContent?: string } };
  }>;
...
async function resolveListingUrl(url: string) { if (!isGoogleGroundingRedirect(url)) return url; const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 5000); try { const response = await fetch(url, { method: "HEAD", redirect: "follow", signal: controller.signal, cache: "no-store", headers: { "User-Agent": "Mozilla/5.0 (compatible; UniversalShoppingSearch/1.0)" } }); const resolved = response.url; if (resolved && !isGoogleGroundingRedirect(resolved) && marketplaceFromUrl(resolved) !== "other") return resolved; } catch {} finally { clearTimeout(timeout); } const controller2 = new AbortController(); const timeout2 = setTimeout(() => controller2.abort(), 5000); try { const response = await fetch(url, { method: "GET", redirect: "follow", signal: controller2.signal, cache: "no-store", headers: { "User-Agent": "Mozilla/5.0 (compatible; UniversalShoppingSearch/1.0)" } }); const resolved = response.url; if (resolved && !isGoogleGroundingRedirect(resolved) && marketplaceFromUrl(resolved) !== "other") return resolved; } catch {} finally { clearTimeout(timeout2); } return null; }
...
function bestGroundingUrl(item: GeminiListing, chunks: GroundingChunk[]) {
  // Never prefer a URL merely because Gemini generated it. Prefer the URL from
  // the cited Google grounding result when sourceIndex identifies one.
  const indexed = item.sourceIndex != null ? chunks[item.sourceIndex]?.web?.uri : undefined;
...
export async function discoverListingsWithGemini(keyword: string, options?: { limit?: number }) {
  ...
  const textUrl = marketplaceUrlFromText(text); const seen = new Set<string>(); const resolvedListings = await Promise.all(parsedListings.map(async (item) => {
    const rawUrl = bestGroundingUrl(item, chunks) ?? textUrl; if (!rawUrl || !item.title) return null;
    const resolvedUrl = await resolveListingUrl(rawUrl); if (!resolvedUrl) return null;
    let url: string; try { url = normalizeProductUrl(resolvedUrl); } catch { return null; }
    const marketplace = marketplaceFromUrl(url); if (marketplace === "other") return null;
    ...
    if (!isProductUrl({ marketplace, url, price })) { console.warn("[gemini-debug] rejected non-product URL", { marketplace, url, title: item.title, sourceIndex: item.sourceIndex }); return null; }
